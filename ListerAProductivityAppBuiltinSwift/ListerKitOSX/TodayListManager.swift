...
class TodayListManager {
    ...
    class func fetchTodayDocumentURLWithCompletionHandler(completionHandler: (url: NSURL?) -> Void) {
    ...
    class func createTodayDocumentURLWithContainerURL(containerURL: NSURL) -> NSURL? {
        ...
        let sampleTodayDocumentURL = NSBundle(forClass: TodayListManager.self).URLForResource("Today", withExtension: AppConfiguration.listerFileExtension)
